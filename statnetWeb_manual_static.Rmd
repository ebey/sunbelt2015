...
**Conditionally uniform models**
  ...
  <div class="ex">
  <font class="ex">Example:</font> `faux.mesa.high` network  
  ![](images/fmh_brgcug.png)  
...
**Covariate effects, odds ratios for tie preferences**
  <ul>
  <li>Rearrange the rows and columns of an adjacency matrix to create a blockmodel where nodes are grouped by values of a covariate. Each subgraph has its own density, which shows potential effects from the covariates.
...
\end{align*}$$
  <li> $\beta_1$, the "effect" of $x_i$, represents the log odds ratio $(y=1|(1,0),(0,0))$  
...
Perhaps some nodes in a directed network are more likely to send ties (more sociable) or more likely to receive ties (more popular). We can capture the heterogeneity of out-degree and in-degree with a row and column effects model (RCE).
  ...
  <div class="ex">
  <font class="ex">Example:</font> Consider a directed network of friendship nominations among a group of monks. Clearly our previous null models do not capture the heterogeneity of in- and out-degree.  
  ![](images/samp_sdinout.png)  
