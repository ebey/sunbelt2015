...
\text{log odds}(y=1) &= \log\left(\frac{p}{1-p}\right),  \hspace{1.5em} 0 < p < 1 & \text{This is sometimes referred to as the logit}
\end{align*}$$
...
Fitting and simulating from this model in R is accomplished in the following way.  
```
Ridx <- matrix(1:n,n,n)
Cidx <- t(Ridx)
...
```
statnetWeb can fit this model with much less hassle.
<font class="ex">insert image</font>
  </div>
  </ul>
