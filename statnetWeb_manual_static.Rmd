...
<ul>
<li>Model summary</li>  
<font class="ex">image here</font>
  <ul>
  Once we have coefficient estimates, we can see how they influence the conditional log-odds of two nodes having a tie:  
  
  $\theta_1 * \delta_1(y_{ij}) + \theta_2 * \delta_2(y_{ij})$  

  For example, if we have a model formula `edges + nodematch("Grade") + degree(0)` with coefficients $\theta_1 = -5$, $\theta_2 = 2$ and $\theta_3 = 1$ respectively, the log-odds of the potential tie below is $(-5*1) + (2*2) + (1*-1) = -2$.
