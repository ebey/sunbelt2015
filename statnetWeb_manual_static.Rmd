...
<li>Conditionally uniform models
  ...
  **add images of each distribution compared to test stat of an observed nw (and add this func to sW)**
  ...
  <ul>
  <li>e.g. A null hypothesis where ties are independent of covariates ($p(Y,x)=p(Y) \times p(x)$) and the ties are drawn from the SRG($n,s$) distribution. We can simulate this by randomly reassigning the ties in $Y$ and holding the covariates $x$ constant.
