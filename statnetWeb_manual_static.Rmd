...
**Conditionally uniform models**
  ...
  <li>BRG: Bernoulli random graphs (simple random graphs)
  <ul>
  ...
  <li>Which $\theta$ to use if we want to reject all bernoulli graphs? If we use a value $\hat\theta$ that maximizes the probability that our observed network $y$ came from the distribution BRG($\theta$) and can reject this model, then we can reject BRG($\theta$) for all $\theta$. It can be shown that the MLE $\hat\theta$ is equal to $\bar{y}$, the density of the observed network.
  </ul>
  <li>CUG($n,s$): All possible graphs conditional on number of ties $s$ (Erdos-Reyni graphs)
  <ul>
  <li>CUG($n,s$) does not require that we estimate $\theta$, it always reproduces the density of the observed network. It often gives similar results to the BRG($\hat\theta$) model above. 
  </ul>
  <div class="ex">
  <font class="ex">Example:</font> Compare the observed density of the `faux.mesa.high` network to the distribution of densities obtained from BRG($\hat\theta$). Note that when testing the standard-deviation of degrees in the network, the null models have similar distributions, but both fail to recreate the level of heterogeneity seen in the observed network.
...
<div align="center">![mesa geodist](images/mesa_geodist_small.png)</div>
  </div>
  <li>We can add more conditions to the model: 
  <ul>
  <li>A null hypothesis where ties are independent of covariates, i.e. $p(Y,x)=p(Y) \times p(x)$, and the ties are drawn from the CUG($n,s$) distribution. We can simulate this by randomly reassigning the ties in $Y$ and holding the covariates $x$ constant.
...
**Covariate effects, odds ratios for tie preferences**
  <ul>
  <li>Rearrange the rows and columns of an adjacency matrix to create a blockmodel where nodes are grouped by values of a covariate. Each subgraph has its own density, which shows potential effects from the covariate.
...
Given differences between densities in the blockmodel, perhaps we think that the values of a nodal attribute are primary factors in tie formation. Use logistic regression to model the effect of a covariate on the likelihood of ties, assuming that ties are independent of each other.
  <ul>
  <li>Given a binary outcome variable $y$ and binary explanatory variable $x$
$$\begin{align*}
 \Pr( y_{i,j} = 1 | x_i,x_j ) &= 
...
\text{log odds}( y_{i,j} = 1 | x_i,x_j ) &= 
  \beta_0+\beta_1 x_i + \beta_2 x_j +  \beta_{12} x_i x_j  
\end{align*}$$
  <li> $\beta_1$ represents the "effect" of $x_i$, the log of the odds ratio $(y=1|(1,0),(0,0))$  
  <li> $\beta_{12}$ represents the effect of the interaction $x_i x_j$, the log of the ratio of odds ratios $(y=1|(1,1),(0,1))/(y=1|(1,0),(0,0))$
...
Perhaps some nodes in a directed network are more likely to send ties (more sociable) or more likely to receive ties (more popular). We can capture the heterogeneity of out-degree and in-degree with a row and column effects model (RCE).
  <ul>
