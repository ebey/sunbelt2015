...
The number of friends that the two red nodes have in common equals the number of triangles that would form if the tie between them existed. In our scenario, the tie on the right is the most likely to form because it closes the most triangles. Notice that the probability of a tie forming does not depend on the state of any non-incident dyads, so this is indeed a Markov graph.
...
You may have noticed that some ergm models take longer to run than others, and that plots are not always available on the MCMC Diagnostics page.
