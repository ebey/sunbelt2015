...
**Covariate effects, odds ratios for tie preferences**
  ...
  $$\begin{align*}
  p_{FF}&=0.0169 \\
  \frac{p_{FM}}{p_{FF}}=\frac{p_{MF}}{p_{FF}}&=0.4002 \\
  \frac{p_{FF}p_{MM}}{p_{FM}p_{MF}}&=3.3178
  \end{align*}$$
  </div>
  </ul>
...
\end{align*}$$
  ...
  $$\begin{align*}
  \text{exp}(\beta_0) &= 0.0172 \\
  \text{exp}(\beta_1) = \text{exp}(\beta_2) &= 0.3961 \\
  \text{exp}(\beta_{12}) &= 3.3595
  \end{align*}$$
...
```
statnetWeb can fit this model with much less hassel.
<font class="ex">insert image</font>
  </div>
  </ul>
