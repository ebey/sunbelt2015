...
<font class="ex">SUMMARY OF FIT</font>
  </div>
  </ul>
...
**$p_1$ model**
The total number of mutual, asymmetric and null dyads in a network can be computed as follows, where $s$ is the total number of ties.
$$\begin{align*}
M &= \sum_{i<j}y_{ij}y_{ji} \\
A &= s - 2M \\
N &= {n \choose 2} - M _ A
\end{align*}$$
Above we saw that in the `sampson` data $M$ was larger than expected under the RCE model. A large observed $M$ when compared to its distribution under some null model or its expected value under some simple conditions indicates high mutuality in the data. Mutuality (or reciprocity) describes the tendency for ties to be mutual. This would indicate within-dyad dependence in the data.
