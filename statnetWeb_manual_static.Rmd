...
What kind of data can be represented as a network?

What can we accomplish with network analysis?  
<ul>
<li>Answer specific research questions</li>
  <ul>
  <li>How might a public health intervention impact the spread of disease?
  <li>
  </ul>
</ul>
  
In order to answer questions like this we will analyze the characteristics of an observed network and then try to fit a model to it.
...
See @snijders2011 for an overview of network analysis methods.
...
Outside of statnetWeb, the nodes and edges of a network can be denoted by adjacency matrices, incidence matrices or edge lists. The network type (directed, valued, etc.) will be reflected in the data.
...
<li>Limitations:
  ...
  <li>Incidence matrices are for relationships between two classes of elements
  </ul>
</ul>
...
See @holland1976 and @wasserman1977 for more on conditionally uniform models.
...
How are $\{\beta_0, \beta_1, \beta_2, \beta_{12}\}$ related to $\{p_{00}, \frac{p_{10}}{p_{00}}, \frac{p_{01}}{p_{00}}, \frac{p_{00}p_{11}}{p_{01}p_{10}}\}$?  
...
<div align="center">![](images/sW_logreg.png) </div>
  ...
  <div align="center">![](images/fmh_logregtests2.png)</div>
...
Why were we able to use the ergm software to fit the models above? Because logistic regression models are ERGMs! Each term in the call to glm corresponds to a term in the ergm formula. Reading the help documentation for each term used above (through the documentation box in statnetWeb or `help("ergm-terms")` in the command line) should present some connections to the logistic regression formula. For example, notice that `nodematch("Sex",diff=TRUE,keep=2)` corresponds to `xr*xc`, but using `nodematch("Sex")` in ergm would be the same as having a dyadic covariate `z <- 1*(xr==xc)` in a glm.

#### Goodness of fit

In the logistic regression models above we chose certain statistics to include in our model, which could have been counts of nodal covariates, dyadic covariates, or functions of these. The coefficients we get after fitting the model are direct results of the values of those statistics in our observed network. This revised workflow image reflects the dependence of the modeling process on the observed network.

![](images/workflow2.png)

So far we have been assessing the fit of our models by comparing the observed value of a test statistic to histograms generated from simulations (draws from the probability distribution) of the model. This allows us to see if the observed value is within the range we would expect to see from the model. If a statistic is included in our model, simulations should always reproduce the observed value - checking this verifies that the model is performing the way we expect it to. If a model is able to reproduce statistics that are not explicitly included in the model, this is a sign that we have captured the key structural features of the data.

statnetWeb has a built in goodness of fit mechanism. For various terms (degree, egdewise shared partners, geodesic distance and more), the user can check statistics and boxplots of simulated values and compared them to the observed network. For example, below is the goodness of fit data for the degree distribution of the `faux.mesa.high ~ edges + nodefactor("Sex") + nodematch("Sex",diff=TRUE,keep=2)` model. The dark black line in the plot corresponds to the degree distribution of `faux.mesa.high`, while the box plots represent the values obtained from simulations from the model.
...
For each node $i$ in a network, let $a_i$ be an effect on $i$'s propensity to send ties and let $b_i$ be an effect on $i$'s propensity to receive ties. The vectors $\vec{a}$ and $\vec{b}$ are called row and column effects, respectively. In a friendship network, $a_i$ can be interpreted as a measure of sociability of node $i$, and $b_j$ can be interpreted as a measure of popularity of node $j$. Note that in an undirected network the row and column sums are equal, so $a_i = b_i$ for each node $i$. Depending on our data, we might want to include either row or column effects in our model. The model below takes into account both row and column effects.
  ...
  $\mu$: log odds of a tie in absence of any row or column effect  
...
<br>
  ...
  In the model, $a_i$ and $b_j$ explicitly account for each node's tendency to have lower or higher than average row and column sums, so we would expect a very good fit for the variability of out- and in-degree. As expected, the fit of in-degree variability is much better compared to the null models, but the out-degree variability predicted by the model is much too high. We do not see any improvement in the fit of the number of reciprocated dyads. Why is the observed heterogeneity in out-degree still lower than expected even after including row effects in our model? 
  
  <div align="center">![](images/samp_rcetests.png) </div>  
  <br>
  
  Using the out-degree term in the goodness of fit allows us to see what is going on in more detail. 
  
  <div align="center">![](images/sW_gofoutdeg.png) </div>
  <br>
  
  In the study design monks were asked on three separate occasions to list their three best friends; there is a tie from node $i$ to $j$ in this network if monk $i$ ever listed monk $j$ as one of his best friends.  This design significantly constrains the out-degree heterogeneity. In our bernoulli model at the beginning of this section, all ties form with the same probability $\text{Pr}(Y=1)=\frac{\text{e}^{\mu}}{1+\text{e}^{\mu}}$ and this results in some level of hetergeneity in out-degrees. Introducing $a_i$ coefficients specific to each node can only increase the out-degree heterogeneity, so the RCE model cannot match the observed variability in out-degrees unless we also introduce constraints. 
...
Holland and Leinhardt (1981) proposed the $p_1$ model to take into consideration "the two elementary social tendencies of reciprocation and differential attraction". The model consists of the same sender and receiver effects as the individual effects model above, with an additional parameter to take into consideration the tendency for ties to be mutual. In this model within dyad relations are dependent, but between dyad relations are still independent of each other; the probability of the tie $Y_{ij}$ depends on $Y_{ji}$, but the probability of $Y_{ik}$ does not depend on either of them.

The model:
$$
\text{Pr}(y_{ij},y_{ji} | \mu, a_i, b_j, \gamma) = \frac{\text{e}^{(\mu + a_i +b_j)y_{ij}+(\mu + a_j + b_i)y_{ji}+\gamma y_{ij}y_{ji}}}{1+\text{e}^{\mu+a_i+b_j}+\text{e}^{\mu+a_j+b_i}+\text{e}^{2\mu+a_i+a_j+b_i+b_j+\gamma}}.
$$

Here $\mu$ represents the overall density, $a_i$ characterizes $i$ as a sender, $b_j$ characterizes $j$ as a receiver, and $\gamma$ is the mutuality parameter.

<div class="ex">
<font class="ex">Example:</font> The $p_1$ model improves our fit of the number of mutual ties in the `samplike` network, but does not improve the fit of any other test statistics.
```
p1fit <- ergm(samplike ~ edges + sender + receiver + mutual)
```

<div align="center">
![](images/samp_p1tests.png)

![](images/samp_p1gof.png)
</div>
...
The two main problems with the $p_1$ model are that it is not generalizable and it has a huge number of parameters. Each individual has both sender and receiver parameters, plus there are two graph level parameters, so we end up with roughly twice as many parameters as nodes in the network. Ideally, we want to find a more general and more parsimonious model.

See @holland1981 ([here](http://www.stat.cmu.edu/~fienberg/Stat36-835/HollandLeinhardt-JASA-1981.pdf)) for more on $p_1$ models.

#### p<sub>2</sub> model 

The $p_2$ model was proposed as an extension to the $p_1$ model allowing for dependence between dyads. The density and mutuality parameters are now allowed to vary over dyads. For each dyad $(i,j)$ we have $\mu_{ij}$ and $\gamma_{ij}$, which are modeled as being linearly related to dyadic covariates $Z_1$ and $Z_2$.

$$\begin{align*}
\mu_{ij} &= \mu + Z_{1ij}\delta_1 \\
\gamma_{ij} &= \gamma + Z_{2ij}\delta_2
\end{align*}$$

Furthermore, the sender and receiver effects are modeled with nodal covariates $X_1$ and $X_2$, weighted by $\theta_1$ and $\theta_2$. The residual terms $A$ and $B$ are modeled as normally distributed random variables with mean $0$ and variances $\sigma_A^2$ and $\sigma_B^2$, respecitively. The parameters $\sigma_A^2$ and $\sigma_B^2$ can be interpreted as the unexplained variance of $a$ and $b$ after taking into account the covariates.

$$\begin{align*}
a_i &= X_{1i}\theta_{1i} + A_i \\
b_j &= X_{2j}\theta_{2j} + B_j
\end{align*}$$

The presence of parameters $A$ and $B$ make this a random effects model, which cannot be fit using the ergm software. The inclusion of sender and receiver covariates is considered the main advantage of the $p_2$ model over $p_1$, but it still has the same drawbacks.

See @vanduijn2004 ([here](http://onlinelibrary.wiley.com/doi/10.1046/j.0039-0402.2003.00258.x/full) and [here](http://www.stats.ox.ac.uk/~snijders/VanDuijnSnijdersZijlstraStatistica2004.pdf)) for more on $p_2$ models.

#### Markov graphs 

As explained in detail in @frank1986, a dependence graph $D$ consists of nodes that correspond to the dyads of a random graph $G$. There is a tie between nodes in $D$ if those dyads in $G$ are conditionally dependent. A Markov graph is defined as a random graph $G$ whose dependence graph $D$ has no ties between disjoint sets of nodes from $G$. In other words, conditional on the rest of the graph, dyads are dependent only if they share a node. In this case, groups of dyads that are all dependent on each other (the cliques of $D$) form triangles and stars in $G$.



<div class="ex">
<font class="ex">Example:</font> To understand the role that dyad dependence can play in the formation of triangles, consider the graph interpretation of the saying that "a friend of my friend is my friend". If having mutual friends makes two people more likely to be friends, the probability of a tie between them depends on the states of other ties.

The number of friends that the two red nodes have in common equals the number of triangles that would be closed if the tie between them formed. In our scenario, the tie on the right is the most likely to form because it closes the most triangles. Notice that the probability of a tie forming does not depend on the state of any dyads except those incident on the tie in question, so this is indeed a Markov graph.

<div align="center">![](images/triangles.png)</div>
</div>

<br>
See @frank1986 ([here](http://www.stat.cmu.edu/~fienberg/Stat36-835/FrankStrauss-JASA-1986.pdf)), @wasserman1996 ([here](http://www.stat.cmu.edu/~brian/780/hw02/WassermanPattison-Psychometrika-1996.pdf)) and @frank1991 for more on Markov graphs.  
...
Exponential-family random graph models are a class of models for specifying the probability distribution for a set of random graphs or networks. Most of the models that we have discussed in this document already are specific forms of ERGMs.
...
<li> $g(y)$: vector of model statistics for network $y$
<li> $\theta$: vector of coefficients for statistics $g(y)$
<li> $k(\theta)$: numerator summed over all possible networks with same node set as $y$ (normalizing constant)
</ul>

Conditional log-odds: $\mathrm{logit}(Y_{ij} = 1|y_{ij}^{c}) = \theta'\delta(y_{ij})$
<ul>
<li>$\delta(y_{ij})$ is the change statistic, i.e. it records how $g(y)$ changes if the tie $y_{ij}$ is toggled on or off.
<li>$\theta$ (the coefficient for a model statistic) is the log-odds of an individual tie conditional on all others.
<li>If $\theta_1$ is positive and $g_1(y)$ is larger when $y_{ij}$ is toggled on, the tie $y_{ij}$ is more likely to form
</ul>
...
In statnetWeb, each model statistic that we want to include is represented by a term in the model formula. There are many terms coded into the statnet software and the term documentation window in statnetWeb gives access to the help file of each one.

<font class="ex">image of term doc box</font>
...
Fitting a model is difficult because calculating the normalizing constant $k(\theta)$ is impossible for all but the simplest models. How does statnetWeb get around this? It depends which terms are in the model formula.

<ul>
<div class="ex">
As we have seen before, model statistics are either dyad dependent or dyad independent. For dyad dependent statistics, the change statistic $\delta(y_{ij})$ depends on the status of dyads in the network other than just $y_{ij}$.  

<font class="ex">Exercise:</font> Which of the following terms are dyad dependent? Why?
...
Formulas with only dyad independent terms:
<ul>
<li>statnetWeb uses an iterative Newton-Raphson algorithm to find the maximum likelihood estimates of the coefficients. This is similar to the process that occurs in logistic regression.
</ul>

Formulas with dyad dependent terms: 
<ul>
<li>statnetWeb uses Markov chain Monte Carlo estimation to iteratively simulate graphs and approximate coefficients that approach the maximum likelihood estimates. With each iteration of this procedure the expected value of a model statistic gets closer to the target value (from the observed network). This, however, is a computationally intensive process and comes with limitations.

<font class="ex">Warning:</font> Problems arise when, for certain sets of parameters, dyadic dependence quickly cascades across the network, causing the distribution of possible graphs to be characterized by two (or more) distinct subsets (regimes) of the outcome space. MCMC simulations will in theory always switch between these regimes, but may spend an arbitrarily long time drawing sequential graphs from one. In this case MCMC is unsuitable to simulate a random draw from the distribution and convergence to the target statistics can be very slow.
</ul>

 

See @hunter2008 ([here](http://www.jstatsoft.org/v24/i03/paper)) for more on ERGM specification and estimation and @snijders2002 ([here](http://www.cmu.edu/joss/content/articles/volume3/Snijders.pdf)) for more on MCMC estimation specifically.

##### Model Output 

<ul>
<li>Model summary</li>  
<div align="center">![](images/modeloutput.png)</div>
  <ul>
  Once we have coefficient estimates, we can see how they influence the conditional log-odds of two nodes having a tie:  
  
  $\theta_1 * \delta_1(y_{ij}) + \theta_2 * \delta_2(y_{ij}) + \theta_3 * \delta_3(y_{ij})$  

  For example, if we have a model formula `edges + degree(0) + nodematch("Grade")` with coefficients $\theta_1 = -5$, $\theta_2 = 2$ and $\theta_3 = 1$ respectively, the log-odds of the potential tie below is $(-5*1) + (2*2) + (1*-1) = -2$.
  
  <div align="center">![](images/modelout.png)</div>
...
A complete model evaluation has two goals, first to ensure that the model itself is not degenerate, and second to see how well the model has captured the key structure of the observed data.

A degenerate model is one whose parameters have been specified in such a way that the distribution of possible graphs exhibits the bimodality discussed in the ERGM Estimation section. Often this manifests as two separate regimes with low and high density graphs.


**MCMC Diagnostics**

For models that require the MCMC estimation procedure (ones with dyadic dependency) statnetWeb offers plots of the MCMC sample statistics compared to the observed statistics. To verify that our model is not degenerate, we want the MCMC sample statistics to vary randomly around the observed values and the difference between the observed and simulated values of the sample statistics to have a roughly bell-shaped distribution, centered at 0. Recent changes in the ergm estimation algorithm mean that these plots can no longer be used to ensure that the mean statistics from the model match the observed network statistics. The goodness-of-fit plots and simulation statistics in statnetWeb are useful for that functionality.

![](images/mcmc_mutual.png)
...
As we have already seen, the user can test how well a model fits the original data by choosing a network statistic that is not in the model, and comparing the value of this statistic observed in the original network to the distribution of values you get in simulated networks from the model.  
When multiple models are saved, compare GOF plots in a chart.

statnetWeb also calculates the AIC and BIC index of every model.
...
Missing data  
Longitudinal network analysis (STERGM)  
