...
Why were we able to use the ergm software to fit the models above? Because logistic regression models are ERGMs! We will make this connection more explicit later, but for now read the help docmentation for each term used above (through the documentation box in statnetWeb or `help("ergm-terms")` in the command line) and connect it to the logistic regression formula.
