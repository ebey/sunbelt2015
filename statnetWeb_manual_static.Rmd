...
<li>Conditionally uniform models
  ...
  <li>Which $\theta$ to use if we want to reject all simple random graphs? Intuitively, if we use a value $\hat\theta$ that maximizes the probability that our observed network $y$ came from the distribution SRG($\theta$) and can reject this model, then we can reject SRG($\theta$) for all $\theta$. It turns out that the MLE $\hat\theta$ is equal to $\bar{y}$, the density of the observed network.
  </ul>
  ...
  **add images of each distribution compared to test stat of an observed nw (and add this func to sW)**
  ...
  <li>(see degree and geodesic distributions in **statnetWeb**)
  </ul>
