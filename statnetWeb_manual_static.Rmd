...
It is not necessary to understand the intricacies of the the ERGM formula yet, we will revisit it in more detail later. We are introducing ERGMs now because many simple models fit into this framework, even though they were developed far before the ERGM theory.
...
BRG($\rho$): Bernoulli random graphs (simple random graphs)
  ...
  <li>BRG($\rho$) is not a good model for most networks, comparing some test statistics the the observed network will allow us to reject this model for this particular $\rho$.
...
CUG($s$): Graphs conditional on number of ties $s$ (Erdos-Reyni graphs)
  <ul>
  <li>All possible networks with $s$ total ties are equally likely (have uniform probability).
...
We can add more conditions to the model: 
  <ul>
...
$$\begin{align*}
p = \Pr(y=1) &= \frac{e^\mu}{1+ e^\mu} &\text{This is sometimes referred to as the expit}\\
\text{odds}(y=1) &= \frac{p}{1-p} \\
\mu = \text{log odds}(y=1) &= \log\left(\frac{p}{1-p}\right),  \hspace{1.5em} 0 < p < 1 & \text{This is sometimes referred to as the logit}
\end{align*}$$

Note that the logit function is the inverse of the logistic function.
$$
\text{logit}(p) = \log\left(\frac{p}{1-p}\right) = \log\left(\frac{e^\mu / (1+e^\mu)}{1/ (1+e^\mu)}\right) = \log(e^\mu) = \mu
$$
...
The simple case of logistic regression where $\mu$ is a constant corresponds to the bernoulli model, where every edge is equally likely.
<ul>
  <div class="ex">
  <font class="ex">Example:</font> We can fit the bernoulli model in R with GLMs:
```
...
![](images/fmh_edges.png)
  ...
  The fit summary tells us that the estimate of $\mu$ is -4.63. This should be the log odds of any edge in the `faux.mesa.high` network. Let's double check:  
  $$\begin{align*}
...
We already know that bernoulli random graphs are not a good model for our dataset (and most others). Logistic regression gives us other options. Given differences between densities in the blockmodel ($\{p_{00}, p_{01}, p_{10}, p_{11}\}$ above), perhaps we think that the values of a nodal attribute are primary factors in tie formation. We can use logistic regression to model the effect of a binary covariate on the likelihood of ties, assuming that ties form independently of each other:
  <ul>
$$\begin{align*}
 \Pr( y_{i,j} = 1 | x_i,x_j ) &= 
...
\end{align*}$$
  ...
  <div class="ex">
  <font class="ex">Example:</font> Compare these odds ratios from `faux.mesa.high` to the relative rates above. The $\beta$ coefficients were obtained from `glm(y ~ xr + xc + xr*xc, family=binomial)` in R, where `xr` is the binary covariate indicating sex in each row, and `xc` is the same for the columns.
  $$\begin{align*}
...
**p<sub>1</sub> model**
The total number of mutual, asymmetric and null dyads in a network can be computed as follows, where $y_{\cdot\cdot}$ is the total number of ties.
$$\begin{align*}
M &= \sum_{i<j}y_{ij}y_{ji} \\
A &= y_{\cdot\cdot} - 2M \\
N &= {n \choose 2} - M _ A
\end{align*}$$
Above we saw that in the `sampson` data $M$ was larger than expected under the RCE model. A large observed $M$ indicates high mutuality in the data. Mutuality (or reciprocity) describes the tendency for ties to be mutual. This would indicate within-dyad dependence in the data, whereas our models so far have assumed that each tie is independent of all other ties.
