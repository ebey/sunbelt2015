...
The [statnet](https://statnet.csde.washington.edu/trac) project provides a comprehensive framework for statistical network modeling based on exponential family random graph models.

As the field grows, more software programs are created for various types of network analysis. A dichotomy has emerged between those accessed with a graphical user interface and those accessed through the command line. While GUI-based programs are easier to _____, 
...
<li>Descriptive</li>
  ...
  <li>"Design-based" methods are used when a sample is drawn from a larger graph
  <li>Statistical inference is limited to the finite population from which the sample is drawn.
  </ul>
<li>Analytical</li>
  <ul>
  <li>"Model-based" methods used when an entire network has been observed.
...
<li>See [Snijders 2011](http://www.stats.ox.ac.uk/~snijders/StatModelsSocNetworks.pdf) for review of various frameworks for statistical network analysis.
</ul>
...
![](images/mesa_degreedist_small.png) ![](images/mesa_degreedist_nullsmall.png)
...
<font class="lobj">Learning objectives:</font> The appropriate model depends on the type of data

- depends on type of data

Recall, we want a model that can capture the underlying structural features of a network with a few parameters.
<ul>
